...
function getCompletedQuizzesCount($pdo, $user_id, $quiz_ids) {
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM userprogress WHERE user_id = ? AND quiz_id IN (' . implode(',', $quiz_ids) . ') AND status = "completed"');
...
function getCompletedTestsCount($pdo, $user_id, $test_ids) {
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM test_attempts WHERE user_id = ? AND test_id IN (' . implode(',', $test_ids) . ') AND score >= 7'); // Assuming passing score is 7
    $stmt->execute([$user_id]);
    return $stmt->fetchColumn();
}
...
function getIncompleteQuizzes($pdo, $user_id, $quiz_ids) {
    $stmt = $pdo->prepare('SELECT id, name FROM quizzes WHERE id IN (' . implode(',', $quiz_ids) . ') AND id NOT IN (SELECT quiz_id FROM userprogress WHERE user_id = ? AND status = "completed")');
    $stmt->execute([$user_id]);
...
function getIncompleteTests($pdo, $user_id, $test_ids) {
    $stmt = $pdo->prepare('SELECT test_id, name FROM tests WHERE test_id IN (' . implode(',', $test_ids) . ') AND test_id NOT IN (SELECT test_id FROM test_attempts WHERE user_id = ? AND score >= 7)');
    $stmt->execute([$user_id]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
<head>
   ...
   .progress-bar-container {
    ...
    margin: 20px 0;
    margin-top: -0px;
    position: relative; /* Ensure the container is positioned relative for absolute positioning of text */
}
...
.progress-bar {
    ...
    border-radius: 25px;
    position: relative; /* Position relative for the text positioning */
    ...
    <?php if ($incomplete_sql_tests): ?>
        ...
        <ul>
            <?php foreach ($incomplete_sql_tests as $test): ?>
                <li><?= htmlspecialchars($test['name']) ?> (ID: <?= $test['test_id'] ?>)</li>
            <?php endforeach; ?>
        </ul>
    <?php endif; ?>
