...
if ($stmt = $conn->prepare($sql)) {
    ...
    if ($stmt->execute()) {
        ...
        while ($row = $result->fetch_assoc()) {
            $questions[$row['question_id']] = $row['expected_keywords'];
        }
        $stmt->close();
    } else {
...
foreach ($questions as $questionId => $expectedKeywords) {
...
<body>
    <div class="container">
        <div class="result">
            <h1>Test Result</h1>
            <p class="score">Score: <?php echo $score; ?>/<?php echo $totalQuestions; ?></p>
            <p class="percentage">Percentage: <?php echo round($percentage, 2); ?>%</p>
